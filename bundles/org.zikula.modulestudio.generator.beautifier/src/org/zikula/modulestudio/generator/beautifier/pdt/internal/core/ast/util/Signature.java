 ...
 * formal type parameters. <i>Rich</i> signatures containing these facets only
...
public final class Signature {
    ...
    private static int checkName(char[] name, char[] typeName, int pos,
        ...
        if (CharOperation.fragmentEquals(name, typeName, pos, true)) {
            ...
            final char currentChar = typeName[pos];
            switch (currentChar) {
                case ' ':
                case '.':
                case '<':
    ...
    public static char[] createArraySignature(char[] typeSignature,
        ...
        final int sigLength = typeSignature.length;
        final char[] result = new char[arrayCount + sigLength];
    ...
    public static char[] createMethodSignature(char[][] parameterTypes,
        ...
        final int returnTypeLength = returnType.length;
        final char[] result = new char[1 + parameterLength + 1
                + returnTypeLength];
        result[0] = C_PARAM_START;
        int index = 1;
        for (int i = 0; i < parameterTypesLength; i++) {
            final char[] parameterType = parameterTypes[i];
            final int length = parameterType.length;
    ...
    public static String createMethodSignature(String[] parameterTypes,
            String returnType) {
        final int parameterTypesLenth = parameterTypes.length;
        final char[][] parameters = new char[parameterTypesLenth][];
        for (int i = 0; i < parameterTypesLenth; i++) {
            parameters[i] = parameterTypes[i].toCharArray();
        }
        return new String(createMethodSignature(parameters,
                returnType.toCharArray()));
    }
    ...
    public static char[] createTypeParameterSignature(char[] typeParameterName,
        ...
        final int length = boundSignatures.length;
        if (length == 0) {
        ...
        final int nameLength = typeParameterName.length;
        final char[] result = new char[nameLength + boundsSize];
        System.arraycopy(typeParameterName, 0, result, 0, nameLength);
        int index = nameLength;
        for (int i = 0; i < length; i++) {
            result[index++] = C_COLON;
            final int boundLength = boundSignatures[i].length;
    ...
    public static String createTypeParameterSignature(String typeParameterName,
        ...
        final int length = boundSignatures.length;
        final char[][] boundSignatureChars = new char[length][];
        for (int i = 0; i < length; i++) {
            boundSignatureChars[i] = boundSignatures[i].toCharArray();
        }
        return new String(createTypeParameterSignature(
                typeParameterName.toCharArray(), boundSignatureChars));
    }
    ...
    public static char[] createCharArrayTypeSignature(char[] typeName,
        ...
        final char[] result = new char[length = buffer.length()];
            ...
            switch (currentChar) {
                ...
                default:
                    if (currentChar == ' '
                        ...
                        final int checkPos = checkNextChar(typeName, '.', pos,
                                length, true);
                        if (checkPos > 0) {
    ...
    private static int encodeTypeSignature(char[] typeName, int start,
            boolean isResolved, int length, StringBuffer buffer) {
        int pos = start;
        pos = consumeWhitespace(typeName, pos, length);
        if (pos >= length) {
            throw new IllegalArgumentException(new String(typeName));
        ...
        switch (currentChar) {
            ...
            case 'b':
                ...
                if (checkPos > 0) {
                    pos = encodeArrayDimension(typeName, checkPos, length,
                            buffer);
            ...
            case '?':
                ...
                if (checkPos > 0) {
                    ...
                    pos = encodeTypeSignature(typeName, checkPos, isResolved,
                            length, buffer);
                    return pos;
                }
    ...
    public static String createTypeSignature(String typeName, boolean isResolved) {
        return createTypeSignature(
                typeName == null ? null : typeName.toCharArray(), isResolved);
    }
    ...
    public static int getArrayCount(char[] typeSignature)
        ...
        try {
            ...
            return count;
        } catch (final ArrayIndexOutOfBoundsException e) { // signature is
                                                           // syntactically
                                                           // incorrect if last
                                                           // character is
                                                           // C_ARRAY
    ...
    public static char[] getElementType(char[] typeSignature)
            throws IllegalArgumentException {
        final int count = getArrayCount(typeSignature);
        if (count == 0) {
            return typeSignature;
        }
        final int length = typeSignature.length;
        final char[] result = new char[length - count];
    ...
    public static int getParameterCount(char[] methodSignature)
        ...
        try {
            ...
            for (;;) {
                ...
                final int e = Util.scanTypeSignature(methodSignature, i);
                if (e < 0) {
                    throw new IllegalArgumentException();
                }
                i = e + 1;
    ...
    public static char[][] getParameterTypes(char[] methodSignature)
            throws IllegalArgumentException {
        try {
            final int count = getParameterCount(methodSignature);
    ...
    public static String[] getParameterTypes(String methodSignature)
            throws IllegalArgumentException {
        final char[][] parameterTypes = getParameterTypes(methodSignature
                .toCharArray());
    ...
    public static String[] getThrownExceptionTypes(String methodSignature)
            throws IllegalArgumentException {
        final char[][] parameterTypes = getThrownExceptionTypes(methodSignature
                .toCharArray());
        return CharOperation.toStrings(parameterTypes);
    }
    ...
    public static char[][] getThrownExceptionTypes(char[] methodSignature)
        ...
        int exceptionStart = CharOperation.indexOf(C_EXCEPTION_START,
                methodSignature);
        if (exceptionStart == -1) {
            final int paren = CharOperation.lastIndexOf(C_PARAM_END,
                    methodSignature);
            if (paren == -1) {
                throw new IllegalArgumentException();
            }
        ...
        while (i < length) {
            ...
            exceptionList.add(CharOperation.subarray(methodSignature,
                    exceptionStart, i));
            exceptionStart = i;
        }
        char[][] result;
    ...
    public static char[][] getTypeArguments(char[] parameterizedTypeSignature)
            throws IllegalArgumentException {
        final int length = parameterizedTypeSignature.length;
        if (length < 2
                || parameterizedTypeSignature[length - 2] != C_GENERIC_END) {
        ...
        while (true) {
            ...
            final char c = parameterizedTypeSignature[p];
            if (c == C_GENERIC_END) {
            ...
            final int e = Util.scanTypeArgumentSignature(
                    parameterizedTypeSignature, p);
            args.add(CharOperation.subarray(parameterizedTypeSignature, p,
                    e + 1));
            p = e + 1;
        }
    }
    ...
    public static String[] getTypeArguments(String parameterizedTypeSignature)
            throws IllegalArgumentException {
        final char[][] args = getTypeArguments(parameterizedTypeSignature
                .toCharArray());
    ...
    public static char[] getTypeErasure(char[] parameterizedTypeSignature)
            ...
            switch (parameterizedTypeSignature[idx]) {
                case C_GENERIC_START:
                    if (deep == 0) {
                        ...
                        System.arraycopy(parameterizedTypeSignature, start,
                                result, pos, size);
                        end = idx;
                        pos += size;
                    }
        ...
        final int size = pos + length - start;
        final char[] resized = new char[size];
    ...
    public static String getTypeErasure(String parameterizedTypeSignature)
            throws IllegalArgumentException {
        return new String(
                getTypeErasure(parameterizedTypeSignature.toCharArray()));
    }
    ...
    public static char[][] getTypeParameters(char[] methodOrTypeSignature)
            throws IllegalArgumentException {
        try {
            final int length = methodOrTypeSignature.length;
            if (length == 0) {
                return CharOperation.NO_CHAR_CHAR;
            }
            ...
            final ArrayList paramList = new ArrayList(1);
            ...
            while (i < length) {
                ...
                while (methodOrTypeSignature[i] == ':') {
                    ...
                    switch (methodOrTypeSignature[i]) {
                        ...
                        case C_RESOLVED:
                            try {
                                i = Util.scanClassTypeSignature(
                                        methodOrTypeSignature, i);
                                i++; // position at start of next param if any
                            } catch (final IllegalArgumentException e) {
                                // not a class type signature -> it is a new
                                // type parameter
                            }
                            break;
                        case C_ARRAY:
                            try {
                                i = Util.scanArrayTypeSignature(
                                        methodOrTypeSignature, i);
                                i++; // position at start of next param if any
                            } catch (final IllegalArgumentException e) {
                                // not an array type signature -> it is a new
                                // type parameter
                            }
                            break;
                        case C_TYPE_VARIABLE:
                            try {
                                i = Util.scanTypeVariableSignature(
                                        methodOrTypeSignature, i);
                                i++; // position at start of next param if any
                            } catch (final IllegalArgumentException e) {
                                // not a type variable signature -> it is a new
                                // type parameter
                            }
                            break;
                ...
                paramList.add(CharOperation.subarray(methodOrTypeSignature,
                        paramStart, i));
    ...
    public static String[] getTypeParameters(String methodOrTypeSignature)
            throws IllegalArgumentException {
        final char[][] params = getTypeParameters(methodOrTypeSignature
                .toCharArray());
    ...
    public static String getTypeVariable(String formalTypeParameterSignature)
            throws IllegalArgumentException {
        return new String(
                getTypeVariable(formalTypeParameterSignature.toCharArray()));
    }
    ...
    public static char[] getTypeVariable(char[] formalTypeParameterSignature)
            throws IllegalArgumentException {
        final int p = CharOperation.indexOf(C_COLON,
                formalTypeParameterSignature);
        if (p < 0) {
    ...
    public static char[][] getTypeParameterBounds(
            ...
            throws IllegalArgumentException {
        final int p1 = CharOperation.indexOf(C_COLON,
                formalTypeParameterSignature);
        if (p1 < 0) {
            // no ":" means can't be a formal type parameter signature
            throw new IllegalArgumentException();
        ...
        final int p2 = CharOperation.indexOf(C_COLON,
                formalTypeParameterSignature, p1 + 1);
        char[] classBound;
        if (p2 < 0) {
            // no interface bounds
        ...
        final char[][] interfaceBounds = CharOperation.splitOn(C_COLON,
                formalTypeParameterSignature, p2 + 1,
                formalTypeParameterSignature.length);
        if (classBound == null) {
            return interfaceBounds;
        }
        final int resultLength = interfaceBounds.length + 1;
    ...
    public static String[] getTypeParameterBounds(
            ...
            throws IllegalArgumentException {
        final char[][] bounds = getTypeParameterBounds(formalTypeParameterSignature
                .toCharArray());
 ...
 */
    public static char[] getQualifier(char[] name) {
        final int firstGenericStart = CharOperation.indexOf(C_GENERIC_START,
                name);
        final int lastDot = CharOperation.lastIndexOf(C_DOT, name, 0,
    ...
    public static String getQualifier(String name) {
        final char[] qualifier = getQualifier(name.toCharArray());
    ...
    public static char[] getReturnType(char[] methodSignature)
        ...
        final int last = Util.scanTypeSignature(methodSignature, paren + 1);
    ...
    public static char[] getSignatureQualifier(char[] typeSignature) {
        ...
        final char[] qualifiedType = Signature.toCharArray(typeSignature);
    ...
    public static char[] getSimpleName(char[] name) {
        ...
        final int nameStart = lastDot < 0 ? 0 : lastDot + 1;
    ...
    private static void appendSimpleName(char[] name, int start, int end,
        ...
        final int nameStart = lastDot < 0 ? start : lastDot + 1;
        final int nameEnd = lastGenericStart < 0 ? end + 1 : lastGenericStart;
 ...
 */
    ...
    public static String removeCapture(String methodOrTypeSignature) {
        final char[] array = methodOrTypeSignature.toCharArray();
        final char[] result = removeCapture(array);
        if (array == result) {
    ...
    public static char[] toCharArray(char[] methodSignature, char[] methodName,
        ...
        return toCharArray(methodSignature, methodName, parameterNames,
    ...
    public static char[] toCharArray(char[] methodSignature, char[] methodName,
            char[][] parameterNames, boolean fullyQualifyTypeNames,
            boolean includeReturnType, boolean isVargArgs) {
        final int firstParen = CharOperation.indexOf(C_PARAM_START,
                methodSignature);
        if (firstParen == -1) {
            throw new IllegalArgumentException();
        }
        ...
        if (includeReturnType) {
            final char[] rts = getReturnType(methodSignature);
        ...
        // parameters
        buffer.append('(');
        final char[][] pts = getParameterTypes(methodSignature);
    ...
    public static char[] toCharArray(char[] signature)
            throws IllegalArgumentException {
        final int sigLength = signature.length;
        ...
        final StringBuffer buffer = new StringBuffer(signature.length + 10);
    ...
    private static int appendArrayTypeSignature(char[] string, int start,
        ...
        final int e = appendTypeSignature(string, index, fullyQualifyTypeNames,
                buffer);
    ...
    private static int appendClassTypeSignature(char[] string, int start,
        ...
        final boolean resolved = (c == C_RESOLVED);
        ...
        while (true) {
            ...
            switch (c) {
                case C_SEMICOLON:
                    ...
                    return p;
                case C_GENERIC_START:
                    final int e = appendTypeArgumentSignatures(string, p,
                            fullyQualifyTypeNames, buffer);
                ...
                case C_DOLLAR:
                    ...
                    if (resolved) {
                         ...
                         * NOTE: This assumes that the type signature is an
                         * inner type signature. This is true in most cases, but
                    ...
                    break;
                default:
                    if (innerTypeStart != -1 && !inAnonymousType
                            && Character.isDigit(c)) {
    ...
    private static int appendTypeArgumentSignatures(char[] string, int start,
        ...
        while (true) {
            ...
            final int e = appendTypeArgumentSignature(string, p,
                    fullyQualifyTypeNames, buffer);
            count++;
            p = e + 1;
        }
        ...
        switch (c) {
            ...
            case C_SUPER:
                ...
                return appendTypeSignature(string, start + 1,
                        fullyQualifyTypeNames, buffer);
            default:
                return appendTypeSignature(string, start,
                        fullyQualifyTypeNames, buffer);
        }
    }
    ...
    public static String toQualifiedName(String[] segments) {
        final int length = segments.length;
        final char[][] charArrays = new char[length][];
        for (int i = 0; i < length; i++) {
    ...
    public static String toString(String methodSignature, String methodName,
            String[] parameterNames, boolean fullyQualifyTypeNames,
            boolean includeReturnType, boolean isVarArgs) {
        char[][] params;
        if (parameterNames == null) {
            params = null;
        }
        else {
            final int paramLength = parameterNames.length;
