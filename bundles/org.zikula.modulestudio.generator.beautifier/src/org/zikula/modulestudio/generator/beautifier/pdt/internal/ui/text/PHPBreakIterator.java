...
import org.eclipse.dltk.internal.ui.text.SequenceCharacterIterator;
 ...
 * @since 3.0
 */
public class PHPBreakIterator extends BreakIterator {
    ...
    public int following(int offset) {
        ...
        final int next = fIterator.following(offset);
        if (next == DONE) {
            return DONE;
        }
        ...
        final Run run = consumeRun(offset);
    ...
    private Run consumeRun(int offset) {
        ...
        final int length = fText.length();
        final Run run = getRun(ch);
