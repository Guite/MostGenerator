...
public class UnaryOperation extends Expression implements IOperationNode {
    ...
    public static final SimplePropertyDescriptor OPERATOR_PROPERTY = new SimplePropertyDescriptor(
            UnaryOperation.class, "operator", Integer.class, MANDATORY); //$NON-NLS-1$
    ...
    public UnaryOperation(int start, int end, AST ast, Expression expr,
            int operator) {
        super(start, end, ast);
    ...
    ASTNode clone0(AST target) {
        final Expression expression = ASTNode.copySubtree(target,
                this.getExpression());
        final UnaryOperation result = new UnaryOperation(this.getStart(),
                this.getEnd(), target, expression, this.getOperator());
        return result;
    }
