...
public class SwitchCase extends Statement {
    ...
    private final ASTNode.NodeList<Statement> actions = new ASTNode.NodeList<Statement>(
            ACTIONS_PROPERTY);
    ...
    public static final ChildPropertyDescriptor VALUE_PROPERTY = new ChildPropertyDescriptor(
            SwitchCase.class, "value", Expression.class, OPTIONAL, CYCLE_RISK); //$NON-NLS-1$
    public static final ChildListPropertyDescriptor ACTIONS_PROPERTY = new ChildListPropertyDescriptor(
            SwitchCase.class, "actions", Statement.class, CYCLE_RISK); //$NON-NLS-1$
    public static final SimplePropertyDescriptor IS_DEFAULT_PROPERTY = new SimplePropertyDescriptor(
            SwitchCase.class, "isDefault", Boolean.class, OPTIONAL); //$NON-NLS-1$
    ...
    public SwitchCase(int start, int end, AST ast, Expression value,
            Statement[] actions, boolean isDefault) {
        super(start, end, ast);
        ...
        for (final Statement statement : actions) {
    ...
    public SwitchCase(int start, int end, AST ast, Expression value,
            List actions, boolean isDefault) {
        this(start, end, ast, value, actions == null ? null
                : (Statement[]) actions.toArray(new Statement[actions.size()]),
                isDefault);
    }
    ...
    public void childrenAccept(Visitor visitor) {
        ...
        for (final ASTNode node : this.actions) {
            node.accept(visitor);
        }
    }
    ...
    public void setValue(Expression value) {
        ...
        final ASTNode oldChild = this.value;
     ...
     * (non-Javadoc)
     * @see
     * org.eclipse.php.internal.core.ast.nodes.ASTNode#internalGetChildListProperty
     * (org.eclipse.php.internal.core.ast.nodes.ChildListPropertyDescriptor)
     */
    @Override
    ...
    ASTNode clone0(AST target) {
        ...
        final SwitchCase result = new SwitchCase(getStart(), getEnd(), target,
                value, actions, isDefault);
        return result;
    }
