...
public final class PHPPairMatcher implements ICharacterPairMatcher {
     ...
     * @since 3.1
     */
    private final boolean fHighlightAngularBrackets = false;
    ...
    private boolean matchPairsAt() {
        ...
        try {
            ...
            final char prevChar = fDocument.getChar(Math.max(fOffset - 1, 0));
    ...
    private int searchForClosingPeer(int offset, char openingPeer,
        ...
        final boolean useGenericsHeuristic = openingPeer == '<';
        if (useGenericsHeuristic && !fHighlightAngularBrackets) {
            return -1;
        }
        final PHPHeuristicScanner scanner = PHPHeuristicScanner
    ...
    private int searchForOpeningPeer(int offset, char openingPeer,
        ...
        final PHPHeuristicScanner scanner = PHPHeuristicScanner
                .createHeuristicScanner(document, offset);
        final int peer = scanner.findOpeningPeer(offset - 1, openingPeer,
                closingPeer);
    ...
    private boolean isTypeParameterBracket(int offset, IDocument document,
        ...
        try {
            ...
            final int prevToken = scanner.previousToken(offset - 1,
                    line.getOffset());
            final int prevTokenOffset = scanner.getPosition() + 1;
            final String previous = prevToken == Symbols.TokenEOF ? null
