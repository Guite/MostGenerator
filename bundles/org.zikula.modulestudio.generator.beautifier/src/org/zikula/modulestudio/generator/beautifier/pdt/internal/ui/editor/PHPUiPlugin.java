...
public class PHPUiPlugin extends AbstractUIPlugin {
    ...
    private final IContentFormatter fActiveFormatter = null;
    ...
    static {
        final String value = Platform
                .getDebugOption("org.eclipse.php.ui/debug"); //$NON-NLS-1$
    ...
    // private PHPFoldingStructureProviderRegistry
    // fFoldingStructureProviderRegistry;
    ...
    private void processCommandLine(BundleContext context) {
        final ServiceTracker environmentTracker = new ServiceTracker(context,
                EnvironmentInfo.class.getName(), null);
        environmentTracker.open();
        final EnvironmentInfo environmentInfo = (EnvironmentInfo) environmentTracker
                .getService();
        environmentTracker.close();
        if (environmentInfo == null) {
            return;
        }
        final String[] args = environmentInfo.getNonFrameworkArgs();
    ...
    // public synchronized MembersOrderPreferenceCache
     ...
     * @since 3.1
     */
    // public synchronized PHPFoldingStructureProviderRegistry
    // getFoldingStructureProviderRegistry() {
    // if (fFoldingStructureProviderRegistry == null) {
    // fFoldingStructureProviderRegistry = new
    // PHPFoldingStructureProviderRegistry();
    // }
    ...
    // final ConfigurationElementSorter sorter = new
    ...
    // final PHPEditorTextHoverDescriptor hoverDescriptor =
    // fPHPEditorTextHoverDescriptors[i];
    // for (int j = i; j > 0; j--) {
     ...
     * @since 3.4
     */
    // public static ISourceModule getEditorInputTypeRoot(IEditorInput
    // editorInput) {
    // Performance: check working copy manager first: this is faster
    // final ISourceModule cu = DLTKUIPlugin.getDefault()
    // .getWorkingCopyManager().getWorkingCopy(editorInput);
    // if (cu != null) {
    // return cu;
    // }
    // final ISourceModule je = (ISourceModule) editorInput
    ...
    // final ElementCreationProxy ecProxy = new ElementCreationProxy(
