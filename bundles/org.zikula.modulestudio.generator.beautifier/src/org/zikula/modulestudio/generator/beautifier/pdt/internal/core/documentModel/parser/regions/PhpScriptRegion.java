package org.zikula.modulestudio.generator.beautifier.pdt.internal.core.documentModel.parser.regions;
 ...
 * 
 * 
 * Based on package org.eclipse.php.internal.core.documentModel.parser.regions;
 * 
 *******************************************************************************/
...
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.documentModel.parser.AbstractPhpLexer;
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.documentModel.parser.PhpLexerFactory;
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.documentModel.parser.Scanner.LexerState;
...
public class PhpScriptRegion extends ForeignRegion implements IPhpScriptRegion {
    ...
    public StructuredDocumentEvent updateRegion(Object requester,
        ...
        try {
            ...
            final int previousIndex = tokensContaier.phpTokens
                    .indexOf(tokenStart) - 1;
            if (previousIndex >= 0) {
                final ITextRegion previousRegion = tokensContaier.phpTokens
                        .get(previousIndex);
            ...
            final ITextRegion[] newTokens = newContainer.getPhpTokens(); // now,
                                                                         // add
            // the new
            // ones
            for (final ITextRegion newToken : newTokens) {
                oldIterator.add(newToken);
            }
    ...
    public void completeReparse(IDocument doc, int start, int length) {
        ...
        final AbstractPhpLexer phpLexer = getPhpLexer(new BlockDocumentReader(
                doc, start, length), null);
        try {
            ST_PHP_LINE_COMMENT = phpLexer.getClass()
                    .getField("ST_PHP_LINE_COMMENT").getInt(phpLexer);
            ST_PHP_IN_SCRIPTING = phpLexer.getClass()
