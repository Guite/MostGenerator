...
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.ast.nodes.ASTParser;
...
public class ASTRewrite {
    ...
    public TextEdit rewriteAST(IDocument document, Map options)
        ...
        final ASTNode rootNode = getRootNode();
        if (rootNode == null) {
            return new MultiTextEdit(); // no changes
        }

        final char[] content = document.get().toCharArray();
        final LineInformation lineInfo = LineInformation.create(document);
        final String lineDelim = TextUtilities
                .getDefaultLineDelimiter(document);

        final ASTNode astRoot = rootNode.getProgramRoot();
        final List commentNodes = astRoot instanceof Program ? ((Program) astRoot)
                .comments() : null;
    ...
    public TextEdit rewriteAST() throws ModelException,
        ...
        final ASTNode root = rootNode.getProgramRoot();
        if (!(root instanceof Program)) {
            throw new IllegalArgumentException(
                    "This API can only be used if the AST is created from a compilation unit or class file"); //$NON-NLS-1$
        }
        final Program astRoot = (Program) root;
        final ISourceModule typeRoot = astRoot.getSourceModule();
        if (typeRoot == null || typeRoot.getBuffer() == null) {
            throw new IllegalArgumentException(
                    "This API can only be used if the AST is created from a compilation unit or class file"); //$NON-NLS-1$
        }

        final char[] content = typeRoot.getBuffer().getCharacters();
        final Document document = new Document(new String(content));
        final LineInformation lineInfo = LineInformation.create(astRoot);
        final String lineDelim = TextUtilities
                .getDefaultLineDelimiter(document);
        final Map options = typeRoot.getScriptProject().getOptions(true);
    ...
    private TextEdit internalRewriteAST(IDocument document, char[] content,
        ...
        final TargetSourceRangeComputer sourceRangeComputer = getExtendedSourceRangeComputer();
        this.eventStore.prepareMovedNodes(sourceRangeComputer);

        final ASTRewriteAnalyzer visitor = new ASTRewriteAnalyzer(ast.lexer(),
    ...
    private ASTNode getRootNode() {
        ...
        for (final Iterator iter = getRewriteEventStore()
                .getChangeRootIterator(); iter.hasNext();) {
            final ASTNode curr = (ASTNode) iter.next();
            if (!RewriteEventStore.isNewNode(curr)) {
                final int currStart = curr.getStart();
                final int currEnd = currStart + curr.getLength();
    ...
    public final void set(ASTNode node, StructuralPropertyDescriptor property,
        ...
        final NodeRewriteEvent nodeEvent = this.eventStore.getNodeEvent(node,
                property, true);
    ...
    private void validatePropertyType(StructuralPropertyDescriptor prop,
        ...
        if (prop.isChildListProperty()) {
            final String message = "Can not modify a list property, use a list rewriter"; //$NON-NLS-1$
            throw new IllegalArgumentException(message);
        }
        // if (node == null) {
    ...
    public final ASTNode createStringPlaceholder(String code, int nodeType) {
        ...
        final ASTNode placeholder = getNodeStore().newPlaceholderNode(nodeType);
        if (placeholder == null) {
            throw new IllegalArgumentException(
    ...
    public final ASTNode createGroupNode(ASTNode[] targetNodes) {
        ...
        final Block res = getNodeStore().createCollapsePlaceholder();
        final ListRewrite listRewrite = getListRewrite(res,
                Block.STATEMENTS_PROPERTY);
        for (final ASTNode targetNode : targetNodes) {
    ...
    private ASTNode createTargetNode(ASTNode node, boolean isMove) {
        ...
        validateIsCorrectAST(node);
        final CopySourceInfo info = getRewriteEventStore().markAsCopySource(
                node.getParent(), node.getLocationInParent(), node, isMove);

        final ASTNode placeholder = getNodeStore().newPlaceholderNode(
                node.getType());
        if (placeholder == null) {
            throw new IllegalArgumentException(
