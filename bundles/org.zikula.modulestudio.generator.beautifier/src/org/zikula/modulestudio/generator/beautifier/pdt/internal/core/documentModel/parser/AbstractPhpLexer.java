...
public abstract class AbstractPhpLexer implements Scanner, PHPRegionTypes {
    ...
    private String removeFromBuffer() {
        final ITextRegion region = bufferedTokens.removeFirst();
     ...
     * 
     * public void setPatterns(IProject project) { if (project != null) { todos
     * = TaskPatternsProvider.getInstance().getPatternsForProject( project); }
     * else { todos = TaskPatternsProvider.getInstance()
    ...
    private void checkForTodo(List<ITextRegion> result, String token,
        ...
        final ArrayList<Matcher> matchers = createMatcherList(comment);
        int startPosition = 0;
    ...
    private ArrayList<Matcher> createMatcherList(String content) {
        final ArrayList<Matcher> list = new ArrayList<Matcher>();// todos.length);
        /*
         * for (int i = 0; i < todos.length; i++) { list.add(i,
         * todos[i].matcher(content)); }
         */
        return list;
    }
    ...
    private Matcher getMinimalMatcher(ArrayList<Matcher> matchers,
        ...
        for (int i = 0; i < size;) {
            final Matcher tmp = matchers.get(i);
