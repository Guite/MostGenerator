...
class Custom {
    ...
    def generate(CustomAction it, Application app, Entity entity, IFileSystemAccess fsa) {
        ...
        if (!app.shouldBeSkipped(templateFilePath)) {
            fsa.generateFile(templateFilePath, customView(it, app, entity, false))
        }
        if (app.generateSeparateAdminTemplates) {
            templateFilePath = templateFile(entity, 'Admin/' + name.formatForCode)
            if (!app.shouldBeSkipped(templateFilePath)) {
                fsa.generateFile(templateFilePath, customView(it, app, entity, true))
            }
        }
    }

    def private customView(CustomAction it, Application app, Entity controller, Boolean isAdmin) '''
        «IF app.generateSeparateAdminTemplates»
        ...
        {% block content %}
            <div class="«app.appName.toLowerCase»-«controller.name.formatForDB» «app.appName.toLowerCase»-«name.formatForDB»">
                <p>Please override this template by moving it from <em>/«app.relativeAppRootPath»/«app.getViewPath»«entity.name.formatForCodeCapital»/«IF isAdmin»Admin/«ENDIF»«name.formatForCode.toFirstLower».html.twig</em> to either <em>/themes/YourTheme/Resources/«app.appName»/views/«entity.name.formatForCodeCapital»/«IF isAdmin»Admin/«ENDIF»«name.formatForCode.toFirstLower».html.twig</em> or <em>/app/Resources/«app.appName»/views/«entity.name.formatForCodeCapital»/«IF isAdmin»Admin/«ENDIF»«name.formatForCode.toFirstLower».html.twig</em>.</p>
            </div>
        {% endblock %}
    '''
}
