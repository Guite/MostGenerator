...
class ViewQuickNavForm {
    ...
    def private categoriesFields(Entity it) '''
        «IF application.targets('1.3.x')»
            ...
            {/if}
        «ELSE»
            {% set categoriesEnabled = featureActivationHelper.isEnabled(constant('«application.vendor.formatForCodeCapital»\\«application.name.formatForCodeCapital»\\Helper\\FeatureActivationHelper::CATEGORIES'), '«name.formatForCode»') %}
