...
class History {
    ...
    def private historyView(Entity it) '''
        ...
        {% endblock %}
        {% block history_table %}
            <form action="{{ path('«app.appName.formatForDB»_«name.formatForDB»_loggablehistory', {«IF hasSluggableFields && slugUnique»slug«ELSE»id«ENDIF»: «name.formatForCode».get«IF hasSluggableFields && slugUnique»Slug«ELSE»Key«ENDIF»()}) }}" method="get">
        ...
        «IF hasIndexAction || hasDetailAction»
            <p>
                {% if isDiffView == true %}
                    ...
                    <a href="{{ path('«appName.formatForDB»_«name.formatForDB»_loggablehistory', {«IF hasSluggableFields && slugUnique»slug«ELSE»id«ENDIF»: «name.formatForCode».get«IF hasSluggableFields && slugUnique»Slug«ELSE»Key«ENDIF»()}) }}" title="{{ linkTitle|e('html_attr') }}"><i class="fas fa-history"></i> {{ linkTitle }}</a>
                {% else %}
                    «IF hasIndexAction»
