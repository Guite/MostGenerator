...
class Categories {
    ...
    def private viewBody(Application it) '''
        ...
        {% for catMapping in obj.categories %}
            <li>{% if catMapping.category.icon %}<i class="fa-fw {{ catMapping.category.icon|e('html_attr') }}"></i> {% endif %}{{ catMapping.category.displayName[app.locale]|default(catMapping.category.name) }}</li>
        {% endfor %}
        </ul>
    '''
