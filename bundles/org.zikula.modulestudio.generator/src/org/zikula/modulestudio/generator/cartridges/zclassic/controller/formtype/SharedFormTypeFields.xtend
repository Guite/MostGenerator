...
class SharedFormTypeFields {
    ...
    def formTypeImports(List<Field> fields, Application app, DataObject dataObject) '''
        ...
        «ENDIF»
        «IF !fields.filter(StringField).filter[role == StringRole.DATE_INTERVAL].empty»
        ...
        «IF !fields.filter(StringField).filter[role == StringRole.PHONE_NUMBER].empty»
        ...
        «ENDIF»
        «IF !fields.filter(StringField).filter[role == StringRole.WEEK].empty»
        ...
        «ENDIF»
        «IF !fields.filter(StringField).filter[role == StringRole.ICON].empty»
    ...
    def definition(DerivedField it) '''«/* No input fields for foreign keys, relations are processed further down */»
        «IF null === entity || entity.getIncomingJoinRelations.filter[r|r.getSourceFields.head == name.formatForDB].empty»
            «IF it instanceof ListField»
                «fetchListEntries»
            «ENDIF»
            «val useCustomSwitch = it instanceof BooleanField»
            ...
            $builder->add(«IF !mandatory && !nullable»$builder->create(«ENDIF»'«name.formatForCode»', «formType»Type::class, [
                ...
                «IF null !== documentation && !documentation.empty»
                    'label_attr' => [
                        'class' => 'tooltips«IF useCustomSwitch» switch-custom«ELSEIF isExpandedListField» «IF (it as ListField).multiple»checkbox«ELSE»radio«ENDIF»-custom«ENDIF»',
                ...
                «ELSEIF isExpandedListField»
                    'label_attr' => [
                        'class' => '«IF (it as ListField).multiple»checkbox«ELSE»radio«ENDIF»-custom',
                    ],
                «ENDIF»
                «helpAttribute»
    ...
    def private helpAttribute(DerivedField it) {
        val messages = helpMessages
        val parameters = helpMessageParameters
