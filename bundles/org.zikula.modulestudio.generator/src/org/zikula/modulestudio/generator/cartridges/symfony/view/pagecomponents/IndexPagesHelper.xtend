    ...
    def docsWithVariables(NamedObject it, Application app) '''
        «IF null !== documentation && !documentation.replaceAll('\\s+', '').empty»
            «IF !documentation.containedTwigVariables.empty»
                <p class="alert alert-info">{% trans with {«documentation.containedTwigVariables.map[v|'\'%' + v + '%\': ' + v + '|default'].join(', ')»} from '«IF it instanceof Entity»«name.formatForCode»«ELSE»messages«ENDIF»' %}«documentation.replace('\'', '\\\'').replaceTwigVariablesForTranslation»{% endtrans %}</p>
            «ELSE»
                <p class="alert alert-info">{% trans from '«IF it instanceof Entity»«name.formatForCode»«ELSE»messages«ENDIF»' %}«documentation.replace('\'', '\\\'')»{% endtrans %}</p>
            «ENDIF»
