...
class TravisFile {
    ...
    def private travisFile(Application it) '''
        ...
        install:
            ...
            - zip -qr «appName».zip .
            «IF targets('1.4-dev')»
            - wget http://ci.zikula.org/job/Zikula_Core-1.4.7/12/artifact/build/archive/Zikula_Core-1.4.7.build12.tar.gz
            - tar -xpzf Zikula_Core-1.4.7.build12.tar.gz
            - rm Zikula_Core-1.4.7.build12.tar.gz
            - cd Zikula_Core-1.4.7
            «ELSE»
            ...
            «IF isSystemModule»
                ...
                - mysql -e "INSERT INTO zk_test.modules (id, name, type, displayname, url, description, directory, version, capabilities, state, securityschema, core_min, core_max) VALUES (NULL, '«appName»', '3', '«name.formatForDisplayCapital»', '«name.formatForDB»', '«IF null !== documentation && documentation != ''»«documentation.replace('"', "'")»«ELSE»«appName» module generated by ModuleStudio «msVersion».«ENDIF»', '«appName»', '«version»', 'N;', '3', 'N;', '1.4.«IF targets('1.4-dev')»7«ELSE»6«ENDIF»', '2.0.0');"
            «ELSE»
                - cd modules
