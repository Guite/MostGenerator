...
class UserRegistrationListener {
    ...
    def generate(Application it) '''
         ...
         * Listener for the `force.registration.approval` event.
         «ENDIF»
         *
         * Occurs when an administrator approves a registration.«IF !targets('3.0')» The UserEntity is the subject.«ENDIF»
         *
         «commonExample.generalEventProperties(it, false)»
         */
        public function forceApproval(«IF targets('3.0')»RegistrationPostApprovedEvent«ELSE»GenericEvent«ENDIF» $event)«IF targets('3.0')»: void«ENDIF»
        {
        }
    '''
}
