...
public class LineCommentEndOffsets {
    ...
    private int[] getOffsets() {
        if (this.offsets == null) {
            if (this.commentList != null) {
                final int nComments = this.commentList.size();
                // count the number of line comments
                int count = 0;
                for (int i = 0; i < nComments; i++) {
                    final Object curr = this.commentList.get(i);
                ...
                for (int i = 0, k = 0; i < nComments; i++) {
                    final Object curr = this.commentList.get(i);
                    if (isLineComment(curr)) {
                        final Comment comment = (Comment) curr;
    ...
    public boolean remove(int offset) {
        final int[] offsetArray = getOffsets(); // returns the shared array
        final int index = Arrays.binarySearch(offsetArray, offset);
        if (index >= 0) {
            if (index > 0) {
