...
public class FieldAccessEvaluator extends GoalEvaluator {
    ...
    private IGoal[] produceNextSubgoal(IGoal previousGoal,
        ...
        final Expression expression = (Expression) typedGoal.getExpression();
        ...
        if (expression instanceof FieldAccess) {
            ...
            final StaticFieldAccess fieldAccess = (StaticFieldAccess) expression;
            receiver = fieldAccess.getDispatcher();
            field = fieldAccess.getField();
        }
        else {
    ...
    @Override
    public IGoal[] init() {
        final IGoal[] goals = produceNextSubgoal(null, null, null);
        if (goals != null) {
            return goals;
        }
    ...
    public IGoal[] subGoalDone(IGoal subgoal, Object result, GoalState state) {
        final IGoal[] goals = produceNextSubgoal(subgoal,
                (IEvaluatedType) result, state);
        if (goals != null) {
            return goals;
        }
        return IGoal.NO_GOALS;
