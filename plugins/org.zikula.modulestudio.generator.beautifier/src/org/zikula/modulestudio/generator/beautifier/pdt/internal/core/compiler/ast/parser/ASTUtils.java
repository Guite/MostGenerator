...
public class ASTUtils {
    ...
    public static VarComment parseVarComment(String content, int start, int end) {
        final Matcher m = VAR_COMMENT_PATTERN.matcher(content);
        if (m.matches()) {
            final int varStart = start + m.group(1).length();
            final String varName = m.group(2);
            final int varEnd = varStart + varName.length();

            final List<TypeReference> typeReferences = new LinkedList<TypeReference>();
            int typeStart = varEnd + m.group(3).length();
            String types = m.group(4);

            int pipeIdx = types.indexOf('|');
            while (pipeIdx >= 0) {
                final String typeName = types.substring(0, pipeIdx);
                final int typeEnd = typeStart + typeName.length();
            ...
            final VariableReference varReference = new VariableReference(
                    varStart, varEnd, varName);
            final VarComment varComment = new VarComment(start, end,
                    varReference,
                    typeReferences.toArray(new TypeReference[typeReferences
                            .size()]));
            return varComment;
        }
        return null;
    ...
    public static ASTNode findMaximalNodeEndingAt(ModuleDeclaration unit,
        ...
        class Visitor extends ASTVisitor {
            ...
            public boolean visitGeneral(ASTNode s) throws Exception {
                ...
                final int sourceEnd = s.sourceEnd();
    ...
    public static IContext findContext(final ISourceModule sourceModule,
        ...
        final ContextFinder visitor = new ContextFinder(sourceModule) {
    ...
    public static FieldDeclaration getConstantDeclaration(
        ...
        final String name = callExpression.getName();
        if ("define".equalsIgnoreCase(name)) {
            final CallArgumentsList args = callExpression.getArgs();
            ...
            moduleDeclaration.traverse(new ASTVisitor() {
                ...
                public boolean visit(Statement s) throws Exception {
                    if (s instanceof UseStatement) {
                        final UseStatement useStatement = (UseStatement) s;
                        for (final UsePart usePart : useStatement.getParts()) {
                            String alias;
            ...
            moduleDeclaration.traverse(new ASTVisitor() {
                ...
                public boolean visit(Statement s) throws Exception {
                    if (s instanceof UseStatement) {
                        final UseStatement useStatement = (UseStatement) s;
                        for (final UsePart usePart : useStatement.getParts()) {
                            final String ns = usePart.getNamespace()
