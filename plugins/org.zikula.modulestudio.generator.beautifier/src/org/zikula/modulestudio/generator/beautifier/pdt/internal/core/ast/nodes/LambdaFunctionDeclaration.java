...
public class LambdaFunctionDeclaration extends Expression {
    ...
    public LambdaFunctionDeclaration(int start, int end, AST ast,
        ...
        final Iterator<FormalParameter> paramIt = formalParameters.iterator();
        while (paramIt.hasNext()) {
            this.formalParameters.add(paramIt.next());
        }
        if (lexicalVars != null) {
            final Iterator<Expression> varsIt = lexicalVars.iterator();
    ...
    public void childrenAccept(Visitor visitor) {
        for (final ASTNode node : this.formalParameters) {
            node.accept(visitor);
        }
        for (final ASTNode node : this.lexicalVariables) {
            node.accept(visitor);
        }
        if (body != null) {
            body.accept(visitor);
