package org.zikula.modulestudio.generator.beautifier.pdt.internal.core.ast.locator;
 ...
 * 
 * 
 * Based on package org.eclipse.php.internal.core.ast.locator;
 * 
 *******************************************************************************/
...
public class PhpElementConciliator {
    ...
    private static boolean isConstant(ASTNode locateNode) {
        ...
        if (locateNode.getType() != ASTNode.SCALAR) {
            final ASTNode parent = locateNode.getParent();
            ASTNode node = null;
            ...
            if ((locateNode instanceof Identifier)
                ...
                final List<Expression> parameters = inv.parameters();
                if (parameters != null && parameters.size() > 0) {
                    final Expression param = parameters.get(0);
    ...
    private static boolean isClassName(ASTNode locateNode) {
        ...
        if (parentType == ASTNode.STATIC_METHOD_INVOCATION
            ...
            final StaticDispatch staticDispatch = (StaticDispatch) parent;
    ...
    private static boolean checkGlobalIdentifier(ASTNode locateNode) {
        ...
        while (parent != null) {
            ...
            if (parent.getType() == ASTNode.FUNCTION_DECLARATION) {
                ...
                final GlobalSeacher searchGlobal = new GlobalSeacher();
    ...
    private static boolean checkGLOBALS(Scalar scalar) {
        ...
        if (scalar.getParent().getType() == ASTNode.ARRAY_ACCESS) {
            final ArrayAccess arrayAccess = (ArrayAccess) scalar.getParent();
    ...
    private static boolean checkGlobal(Identifier targetIdentifier,
        ...
        for (final Variable current : variables) {
            ...
            if (current.getName().getType() == ASTNode.IDENTIFIER) {
                final Identifier id = (Identifier) current.getName();
    ...
    private static class DefinedSearcher extends ApplyAll {
        ...
        public boolean apply(ASTNode node) {
            ...
            if (node.getType() == ASTNode.SCALAR) {
                ...
                final FunctionInvocation functionInvocation = (FunctionInvocation) node;
        ...
        public boolean apply(ASTNode node) {
            ...
            if (node.getType() == ASTNode.VARIABLE) {
                ...
                if (variable.isDollared()) {
                    ...
                    final Identifier identifier = (Identifier) variable
                            .getName();
    ...
    private static class FunctionSearcher extends ApplyAll {
        ...
        public boolean apply(ASTNode node) {
            ...
            if (node.getType() == ASTNode.CLASS_DECLARATION) {
                ...
                final FunctionDeclaration functionDeclaration = (FunctionDeclaration) node;
                final Identifier identifier = functionDeclaration
                        .getFunctionName();
        ...
        public boolean apply(ASTNode node) {
            ...
            if (node.getType() == ASTNode.CLASS_DECLARATION
                ...
                if (identifier.getParent().getType() == ASTNode.VARIABLE) {
                    final Variable variable = (Variable) identifier.getParent();
                ...
                for (final Variable variable : variables) {
                    ...
                    if (variable.isDollared()
                        ...
                        final Identifier identifier = (Identifier) variableName;
