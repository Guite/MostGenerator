...
public class ForStatement extends Statement {
    ...
    private final ASTNode.NodeList<Expression> conditions = new ASTNode.NodeList<Expression>(
            EXPRESSION_PROPERTY);
    private final ASTNode.NodeList<Expression> updaters = new ASTNode.NodeList<Expression>(
            UPDATERS_PROPERTY);
    ...
    public static final ChildPropertyDescriptor BODY_PROPERTY = new ChildPropertyDescriptor(
            ForStatement.class, "body", Statement.class, MANDATORY, CYCLE_RISK); //$NON-NLS-1$
    ...
    private ForStatement(int start, int end, AST ast,
            Expression[] initializations, Expression[] conditions,
            Expression[] increasements, Statement action) {
        super(start, end, ast);

        if (initializations == null || conditions == null
                || increasements == null || action == null) {
            throw new IllegalArgumentException();
        }
        for (final Expression init : initializations) {
            this.initializers.add(init);
        }
        for (final Expression cond : conditions) {
            this.conditions.add(cond);
        }
        for (final Expression inc : increasements) {
            this.updaters.add(inc);
        }
        setBody(action);
    }

    public ForStatement(int start, int end, AST ast, List initializations,
            List conditions, List increasements, Statement action) {
        this(start, end, ast, initializations == null ? null
                : (Expression[]) initializations
                        .toArray(new Expression[initializations.size()]),
                conditions == null ? null : (Expression[]) conditions
                        .toArray(new Expression[conditions.size()]),
                increasements == null ? null : (Expression[]) increasements
    ...
    public void childrenAccept(Visitor visitor) {
        for (final ASTNode node : this.initializers) {
            node.accept(visitor);
        }
        for (final ASTNode node : this.conditions) {
            node.accept(visitor);
        }
        for (final ASTNode node : this.updaters) {
            node.accept(visitor);
        }
        body.accept(visitor);
    }
    ...
    ASTNode clone0(AST target) {
        ...
        final ForStatement result = new ForStatement(this.getStart(),
                this.getEnd(), target, inits, conds, updtaters, body);
        return result;
    }
