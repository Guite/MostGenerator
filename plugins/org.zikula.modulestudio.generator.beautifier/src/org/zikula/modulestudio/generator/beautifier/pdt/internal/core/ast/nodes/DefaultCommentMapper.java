...
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.ast.visitor.ApplyAll;
...
public class DefaultCommentMapper {
    ...
    Comment getComment(int position) {
        ...
        final int size = this.comments.length;
        if (size == 0) {
            return null;
        }
        final int index = getCommentIndex(0, position, 0);
        if (index < 0) {
            return null;
        }
    ...
    private int getCommentIndex(int start, int position, int exact) {
        ...
        while (bottom <= top) {
            ...
            final int commentStart = comment.getStart();
    ...
    public int getExtendedEnd(ASTNode node) {
        ...
        if (this.trailingPtr >= 0) {
            ...
            if (range >= 0) {
                final Comment lastComment = this.comments[(int) range];
    ...
    void initialize(Program unit, AstLexer sc, IDocument document) {
        ...
        // Start unit visit
        final AbstractVisitor commentVisitor = new CommentMapperVisitor();
        unit.accept(commentVisitor);

        // Reduce leading arrays if necessary
        final int leadingCount = this.leadingPtr + 1;
        ...
        if (this.trailingPtr >= 0) {
            ...
            // reduce array size
            final int trailingCount = this.trailingPtr + 1;
            if (trailingCount > 0
    ...
    int storeLeadingComments(ASTNode node, int previousEnd,
        ...
        final int nodeStart = node.getStart();
        int extended = nodeStart;

        // Get line of node start position
        final int previousEndLine = getLineNumber(previousEnd, parentLineRange);
        final int nodeStartLine = getLineNumber(nodeStart, parentLineRange);
        ...
        while (idx >= 0 && previousStart >= previousEnd) {
            ...
            final Comment comment = this.comments[idx];
            final int commentStart = comment.getStart();
            final int end = commentStart + comment.getLength() - 1;
            final int commentLine = getLineNumber(commentStart, parentLineRange);
            if (end <= previousEnd
                ...
                try {
                    ...
                    final String token = this.scanner.yytext();
        ...
        if (startIdx != -1) {
            ...
            if (startIdx <= endIdx) {
                if (++this.leadingPtr == 0) {
                    ...
                    final int newLength = (this.leadingPtr * 3 / 2)
                            + STORAGE_INCREMENT;
                    System.arraycopy(this.leadingNodes, 0,
    ...
    int storeTrailingComments(ASTNode node, int nextStart, boolean lastChild,
        ...
        // Init extended position
        final int nodeEnd = node.getStart() + node.getLength() - 1;
        if (nodeEnd == nextStart) {
            ...
            if (++this.trailingPtr == 0) {
                ...
                final int newLength = (this.trailingPtr * 3 / 2)
                        + STORAGE_INCREMENT;
                System.arraycopy(this.trailingNodes, 0,
        ...
        // Get line number
        final int nodeEndLine = getLineNumber(nodeEnd, parentLineRange);
        ...
        int endIdx = -1;
        final int length = this.comments.length;
        ...
        if (endIdx != -1) {
            ...
            if (!lastChild) {
                final int nextLine = getLineNumber(nextStart, parentLineRange);
                final int previousLine = getLineNumber(previousEnd,
                        parentLineRange);
            ...
            this.trailingNodes[this.trailingPtr] = node;
            final long nodeRange = (((long) startIdx) << 32) + endIdx;
            ...
            int ptr = this.trailingPtr - 1; // children extended end were stored
                                            // before
            while (ptr >= 0) {
                final long range = this.trailingIndexes[ptr];
                if (range != -1) {
                    break; // there's no more unresolved nodes
                }
                final ASTNode unresolved = this.trailingNodes[ptr];
            ...
            if (ptr > this.lastTrailingPtr) {
                final int offset = ptr - this.lastTrailingPtr;
    ...
    class CommentMapperVisitor extends ApplyAll {
        ...
        protected boolean apply(ASTNode node) {
            ...
            final ASTNode sibling = parent == this.topSiblingParent ? (ASTNode) this.siblings[this.siblingPtr]
                    : null;
            if (sibling != null) {
            ...
            final int[] previousLineRange = this.siblingPtr > -1 ? this.parentLineRange[this.siblingPtr]
                    : new int[] {
                            1,
            ...
            if (this.topSiblingParent != parent) {
                ...
                if (this.topSiblingParent == null) {
                    ...
                    final int parentStart = parent.getStart();
                    final int firstLine = getLineNumber(parentStart,
                            previousLineRange);
                    final int lastLine = getLineNumber(
                    ...
                    if (this.parentLineRange[this.siblingPtr] == null) {
                        ...
                        final int[] lineRange = this.parentLineRange[this.siblingPtr];
        ...
        public void endVisit(ASTNode node) {
            ...
            final ASTNode sibling = this.topSiblingParent == node ? (ASTNode) this.siblings[this.siblingPtr]
                    : null;
            if (sibling != null) {
                try {
