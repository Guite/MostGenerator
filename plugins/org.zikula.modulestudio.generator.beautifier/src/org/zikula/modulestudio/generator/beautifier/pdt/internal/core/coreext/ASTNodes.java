...
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.ast.rewrite.ASTRewriteFlattener;
...
public class ASTNodes {
    ...
    public static String asString(ASTNode node) {
        final ASTRewriteFlattener flattener = new ASTRewriteFlattener(null);
    ...
    public static boolean needsParentheses(Expression expression) {
        final int type = expression.getType();
    ...
    public static ASTNode findParent(ASTNode node,
        ...
        for (final StructuralPropertyDescriptor[] path : pathes) {
            ASTNode current = node;
            int d = path.length - 1;
            for (; d >= 0 && current != null; d--) {
                final StructuralPropertyDescriptor descriptor = path[d];
