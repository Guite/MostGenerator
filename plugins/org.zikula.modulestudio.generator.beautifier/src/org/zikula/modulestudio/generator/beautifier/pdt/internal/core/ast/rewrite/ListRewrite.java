...
public final class ListRewrite {
    ...
    public void remove(ASTNode node, TextEditGroup editGroup) {
        ...
        final RewriteEvent event = getEvent().removeEntry(node);
        if (editGroup != null) {
    ...
    public void replace(ASTNode node, ASTNode replacement,
        ...
        final RewriteEvent event = getEvent().replaceEntry(node, replacement);
        if (editGroup != null) {
            getRewriteStore().setEventEditGroup(event, editGroup);
        }
    }
    ...
    public void insertAfter(ASTNode node, ASTNode element,
        ...
        final int index = getEvent().getIndex(element, ListRewriteEvent.BOTH);
        if (index == -1) {
    ...
    private void internalInsertAt(ASTNode node, int index,
        ...
        final RewriteEvent event = getEvent().insert(node, index);
        if (boundToPrevious) {
    ...
    private ASTNode createTargetNode(ASTNode first, ASTNode last,
        ...
        final NodeInfoStore nodeStore = this.rewriter.getNodeStore();
        final ASTNode placeholder = nodeStore.newPlaceholderNode(first
                .getType()); // revisit:
                             // could
                             // use
                             // list
        ...
        final Block internalPlaceHolder = nodeStore.createCollapsePlaceholder();
        final CopySourceInfo info = getRewriteStore().createRangeCopy(
                this.parent, this.childProperty, first, last, isMove,
    ...
    public List getOriginalList() {
        final List list = (List) getEvent().getOriginalValue();
    ...
    public List getRewrittenList() {
        final List list = (List) getEvent().getNewValue();
        return Collections.unmodifiableList(list);
    }
