...
import org.zikula.modulestudio.generator.beautifier.pdt.internal.core.ast.util.Signature;
...
public final class RewriteEventStore {
    ...
    public static class CopySourceInfo implements Comparable {
        ...
        public int compareTo(Object o2) {
            final CopySourceInfo r2 = (CopySourceInfo) o2;

            final int startDiff = this.getNode().getStart()
                    - r2.getNode().getStart();
            if (startDiff != 0) {
    ...
    private static class NodeRangeInfo implements Comparable {
        ...
        @Override
        public int compareTo(Object o2) {
            final NodeRangeInfo r2 = (NodeRangeInfo) o2;

            final int startDiff = this.getStartNode().getStart()
                    - r2.getStartNode().getStart();
            if (startDiff != 0) {
                return startDiff; // insert before if start node is first
            }
            final int endDiff = this.getEndNode().getStart()
        ...
        public void updatePlaceholderSourceRanges(
            ...
            final TargetSourceRangeComputer.SourceRange startRange = sourceRangeComputer
                    .computeSourceRange(getStartNode());
            final TargetSourceRangeComputer.SourceRange endRange = sourceRangeComputer
                    .computeSourceRange(getEndNode());
            final int startPos = startRange.getStartPosition();
            final int endPos = endRange.getStartPosition()
                    + endRange.getLength();

            final Block internalPlaceholder = getInternalPlaceholder();
    ...
    public void addEvent(ASTNode parent,
        ...
        final EventHolder holder = new EventHolder(parent, childProperty, event);

        List entriesList = (List) this.eventLookup.get(parent);
        if (entriesList != null) {
            for (int i = 0; i < entriesList.size(); i++) {
                final EventHolder curr = (EventHolder) entriesList.get(i);
    ...
    public RewriteEvent getEvent(ASTNode parent,
        ...
        final List entriesList = (List) this.eventLookup.get(parent);
        if (entriesList != null) {
            for (int i = 0; i < entriesList.size(); i++) {
                final EventHolder holder = (EventHolder) entriesList.get(i);
    ...
    public NodeRewriteEvent getNodeEvent(ASTNode parent,
        ...
        if (event == null && forceCreation) {
            final Object originalValue = accessOriginalValue(parent,
                    childProperty);
    ...
    public ListRewriteEvent getListEvent(ASTNode parent,
        ...
        ListRewriteEvent event = (ListRewriteEvent) getEvent(parent,
                childProperty);
        if (event == null && forceCreation) {
            final List originalValue = (List) accessOriginalValue(parent,
                    childProperty);
    ...
    public PropertyLocation getPropertyLocation(Object value, int kind) {
        for (final Iterator iter = this.eventLookup.values().iterator(); iter
                .hasNext();) {
            final List events = (List) iter.next();
            for (int i = 0; i < events.size(); i++) {
                final EventHolder holder = (EventHolder) events.get(i);
                final RewriteEvent event = holder.event;
                ...
                if (event.isListRewrite()) {
                    ...
                    for (final RewriteEvent element : children) {
                        if (isNodeInEvent(element, value, kind)) {
                            return new PropertyLocation(holder.parent,
                                    holder.childProperty);
                        }
                    }
    ...
    public RewriteEvent findEvent(Object value, int kind) {
        for (final Iterator iter = this.eventLookup.values().iterator(); iter
            ...
            final List events = (List) iter.next();
            for (int i = 0; i < events.size(); i++) {
                final RewriteEvent event = ((EventHolder) events.get(i)).event;
                if (isNodeInEvent(event, value, kind)) {
                    return event;
                }
                if (event.isListRewrite()) {
    ...
    public int getChangeKind(ASTNode node) {
        final RewriteEvent event = findEvent(node, ORIGINAL);
        if (event != null) {
    ...
    private final CopySourceInfo createCopySourceInfo(
        ...
        final CopySourceInfo copySource = new CopySourceInfo(location, node,
                isMove);
    ...
    public final CopySourceInfo createRangeCopy(ASTNode parent,
        ...
        final CopySourceInfo copyInfo = createCopySourceInfo(null,
                internalPlaceholder, isMove);
        internalPlaceholder.setProperty(INTERNAL_PLACEHOLDER_PROPERTY,
                internalPlaceholder);

        final NodeRangeInfo copyRangeInfo = new NodeRangeInfo(parent,
                childProperty, first, last, copyInfo, replacingNode, editGroup);

        final ListRewriteEvent listEvent = getListEvent(parent, childProperty,
                true);

        final int indexFirst = listEvent.getIndex(first, ListRewriteEvent.OLD);
        if (indexFirst == -1) {
            throw new IllegalArgumentException(
                    "Start node is not a original child of the given list"); //$NON-NLS-1$
        }
        final int indexLast = listEvent.getIndex(last, ListRewriteEvent.OLD);
        if (indexLast == -1) {
            throw new IllegalArgumentException(
        ...
        final PropertyLocation loc = new PropertyLocation(parent, childProperty);
    ...
    public CopySourceInfo[] internalGetCopySources(List copySources,
            ASTNode node) {
        final ArrayList res = new ArrayList(3);
        for (int i = 0; i < copySources.size(); i++) {
            final CopySourceInfo curr = (CopySourceInfo) copySources.get(i);
        ...
        final CopySourceInfo[] arr = (CopySourceInfo[]) res
    ...
    private void assertNoOverlap(ListRewriteEvent listEvent, int indexFirst,
        ...
        for (final Iterator iter = innerList.iterator(); iter.hasNext();) {
            final NodeRangeInfo curr = (NodeRangeInfo) iter.next();
            final int currStart = listEvent.getIndex(curr.getStartNode(),
                    ListRewriteEvent.BOTH);
            final int currEnd = listEvent.getIndex(curr.getEndNode(),
                    ListRewriteEvent.BOTH);
    ...
    private void removeMoveRangePlaceholders() {
        for (final Iterator iter = this.nodeRangeInfos.entrySet().iterator(); iter
                .hasNext();) {
            final Map.Entry entry = (Map.Entry) iter.next();
            final Set placeholders = new HashSet(); // collect all placeholders
            final List rangeInfos = (List) entry.getValue(); // list of
            // CopySourceRange
            ...
            final PropertyLocation loc = (PropertyLocation) entry.getKey();

            final RewriteEvent[] children = getListEvent(loc.getParent(),
                    loc.getProperty(), true).getChildren();
            final List revertedChildren = new ArrayList();
            revertListWithRanges(children, placeholders, revertedChildren);
            final RewriteEvent[] revertedChildrenArr = (RewriteEvent[]) revertedChildren
    ...
    private void revertListWithRanges(RewriteEvent[] childEvents,
        ...
        for (final RewriteEvent event : childEvents) {
            final ASTNode node = (ASTNode) event.getOriginalValue();
            if (placeholders.contains(node)) {
                final RewriteEvent[] placeholderChildren = getListEvent(node,
    ...
    private void prepareNodeRangeCopies(
        ...
        for (final Iterator iter = this.nodeRangeInfos.entrySet().iterator(); iter
            ...
            final RewriteEvent[] newChildren = processListWithRanges(
                    rangeInfos, children, sourceRangeComputer);
            addEvent(loc.getParent(), loc.getProperty(), new ListRewriteEvent(
    ...
    private RewriteEvent[] processListWithRanges(List rangeInfos,
        ...
        final Stack newChildrenStack = new Stack();
        final Stack topInfoStack = new Stack();

        final Iterator rangeInfoIterator = rangeInfos.iterator();
        ...
        for (final RewriteEvent event : childEvents) {
            ...
            while (nextInfo != null && node == nextInfo.getStartNode()) { // is
                ...
                final Block internalPlaceholder = nextInfo
                        .getInternalPlaceholder();
            ...
            while (topInfo != null && node == topInfo.getEndNode()) {
                final RewriteEvent[] placeholderChildEvents = (RewriteEvent[]) newChildEvents
                        .toArray(new RewriteEvent[newChildEvents.size()]);
                final Block internalPlaceholder = topInfo
                        .getInternalPlaceholder();
    ...
    private void prepareSingleNodeCopies() {
        for (int i = 0; i < this.nodeCopySources.size(); i++) {
            final CopySourceInfo curr = (CopySourceInfo) this.nodeCopySources
                    .get(i);
    ...
    private void doMarkMovedAsRemoved(CopySourceInfo curr, ASTNode parent,
        ...
        if (childProperty.isChildListProperty()) {
            final ListRewriteEvent event = getListEvent(parent, childProperty,
                    true);
            final int index = event.getIndex(curr.getNode(),
                    ListRewriteEvent.OLD);
            if (index != -1
            ...
            final NodeRewriteEvent event = getNodeEvent(parent, childProperty,
                    true);
    ...
    private void validateIsListProperty(StructuralPropertyDescriptor property) {
        if (!property.isChildListProperty()) {
            final String message = property.getId() + " is not a list property"; //$NON-NLS-1$
    ...
    private void validateHasChildProperty(ASTNode parent,
        ...
        if (!parent.structuralPropertiesForType().contains(property)) {
            final String message = Signature.getSimpleName(parent.getClass()
                    .getName()) + " has no property " + property.getId(); //$NON-NLS-1$
            throw new IllegalArgumentException(message);
        }
    }
    ...
    private void validateIsNodeProperty(StructuralPropertyDescriptor property) {
        if (property.isChildListProperty()) {
            final String message = property.getId() + " is not a node property"; //$NON-NLS-1$
            throw new IllegalArgumentException(message);
        }
    }
