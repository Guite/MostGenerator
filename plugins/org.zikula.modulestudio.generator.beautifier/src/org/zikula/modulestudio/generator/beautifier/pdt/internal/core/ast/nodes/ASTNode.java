...
public abstract class ASTNode implements Visitable {
    ...
    public final Object getStructuralProperty(
        ...
        if (property instanceof SimplePropertyDescriptor) {
            final SimplePropertyDescriptor p = (SimplePropertyDescriptor) property;
            if (p.getValueType() == Integer.class) {
                final int result = internalGetSetIntProperty(p, true, 0);
                return new Integer(result);
            }
            else if (p.getValueType() == Boolean.class) {
                final boolean result = internalGetSetBooleanProperty(p, true,
                        false);
    ...
    public final void setStructuralProperty(
        ...
        if (property instanceof SimplePropertyDescriptor) {
            ...
            if (p.getValueType() == int.class) {
                final int arg = ((Integer) value).intValue();
                internalGetSetIntProperty(p, false, arg);
                return;
            }
            else if (p.getValueType() == boolean.class) {
                final boolean arg = ((Boolean) value).booleanValue();
        ...
        if (property instanceof ChildPropertyDescriptor) {
            final ChildPropertyDescriptor p = (ChildPropertyDescriptor) property;
            final ASTNode child = (ASTNode) value;
    ...
    public final ASTNode getRoot() {
        ...
        while (true) {
            final ASTNode p = candidate.getParent();
            if (p == null) {
    ...
    public final void delete() {
        final StructuralPropertyDescriptor p = getLocationInParent();
        if (p == null) {
            // node is unparented
            return;
        }
        ...
        if (p.isChildListProperty()) {
            final List l = (List) getParent().getStructuralProperty(
                    this.location);
            l.remove(this);
        }
    }
    ...
    public final Object getProperty(String propertyName) {
        ...
        final Map m = (Map) this.property1;
    ...
    public static <T extends ASTNode> T copySubtree(AST target, T node) {
        ...
        final T newNode = (T) node.clone(target);
        return newNode;
    }
    ...
    public static List copySubtrees(AST target, List nodes) {
        final List result = new ArrayList(nodes.size());
        for (final Iterator it = nodes.iterator(); it.hasNext();) {
            final ASTNode oldNode = (ASTNode) it.next();
            final ASTNode newNode = oldNode.clone(target);
            result.add(newNode);
        }
        return result;
    }
    ...
    final ASTNode clone(AST target) {
        ...
        final ASTNode c = this.clone0(target);
    ...
    static void checkNewChild(ASTNode node, ASTNode newChild,
        ...
        final Class childClass = newChild.getClass();
    ...
    class NodeList<T extends ASTNode> extends AbstractList<T> {
        ...
        class Cursor implements Iterator<T> {
            ...
            @Override
            public T next() {
                final T result = NodeList.this.store.get(this.position);
                this.position++;
                return result;
            }
        ...
        public T set(int index, T element) {
            ...
            final ASTNode newChild = element;
            final ASTNode oldChild = this.store.get(index);
            ...
            final T result = this.store.set(index, (T) newChild);
        ...
        public T remove(int index) {
            ...
            // n.b. setParent will call ast.modifying()
            oldChild.setParent(null, null);
            final T result = this.store.remove(index);
            updateCursors(index, -1);
        ...
        Cursor newCursor() {
            synchronized (this) {
                ...
                final Cursor result = new Cursor();
        ...
        private void updateCursors(int index, int delta) {
            ...
            for (final Object element : this.cursors) {
                final Cursor c = (Cursor) element;
