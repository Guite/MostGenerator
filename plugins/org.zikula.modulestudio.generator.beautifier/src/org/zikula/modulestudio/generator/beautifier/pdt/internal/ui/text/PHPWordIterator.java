...
public class PHPWordIterator extends BreakIterator {
     ...
     */
    private final PHPBreakIterator fIterator;
    ...
    public int preceding(int offset) {
        final int first = fIterator.preceding(offset);
        if (isWhitespace(first, offset)) {
            final int second = fIterator.preceding(first);
    ...
    public int following(int offset) {
        final int first = fIterator.following(offset);
        if (eatFollowingWhitespace(offset, first)) {
            final int second = fIterator.following(first);
    ...
    private boolean isDelimiter(int offset, int exclusiveEnd) {
        ...
        final CharSequence seq = fIterator.fText;

        while (offset < exclusiveEnd) {
            final char ch = seq.charAt(offset);
            if (ch != '\n' && ch != '\r') {
                return false;
            }
            offset++;
