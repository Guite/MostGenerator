...
public class DeclareStatement extends Statement {
    ...
    public static final ChildListPropertyDescriptor DIRECTIVE_NAMES_PROPERTY = new ChildListPropertyDescriptor(
            ...
            "directiveNames", Identifier.class, NO_CYCLE_RISK); //$NON-NLS-1$
    public static final ChildListPropertyDescriptor DIRECTIVE_VALUES_PROPERTY = new ChildListPropertyDescriptor(
            DeclareStatement.class,
            "directiveValues", Expression.class, NO_CYCLE_RISK); //$NON-NLS-1$
    ...
    private DeclareStatement(int start, int end, AST ast,
            Identifier[] directiveNames, Expression[] directiveValues,
            Statement action) {
        super(start, end, ast);

        if (directiveNames == null || directiveValues == null
                || directiveNames.length != directiveValues.length) {
            throw new IllegalArgumentException();
        }
        for (final Identifier identifier : directiveNames) {
            this.directiveNames.add(identifier);
        }
        for (final Expression expression : directiveValues) {
            this.directiveValues.add(expression);
        }
        setBody(action);
    }

    public DeclareStatement(int start, int end, AST ast, List directiveNames,
            List directiveValues, Statement action) {
        this(start, end, ast, directiveNames == null ? null
                : (Identifier[]) directiveNames
                        .toArray(new Identifier[directiveNames.size()]),
                directiveValues == null ? null : (Expression[]) directiveValues
                        .toArray(new Expression[directiveValues.size()]),
                action);
    }
    ...
    ASTNode clone0(AST target) {
        ...
        final List values = ASTNode
                .copySubtrees(target, this.directiveValues());
        final Statement body = ASTNode.copySubtree(target, getBody());

        final DeclareStatement echoSt = new DeclareStatement(this.getStart(),
                this.getEnd(), target, names, values, body);
        return echoSt;
    }
