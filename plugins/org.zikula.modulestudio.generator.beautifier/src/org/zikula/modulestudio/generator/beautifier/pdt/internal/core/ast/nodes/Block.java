...
public class Block extends Statement {
    ...
    private Block(int start, int end, AST ast, Statement[] statements,
            boolean isCurly) {
        super(start, end, ast);
    ...
    public Block(int start, int end, AST ast, List statements, boolean isCurly) {
        this(start, end, ast, statements == null ? null
                : (Statement[]) statements.toArray(new Statement[statements
                        .size()]), isCurly);
    }
    ...
    ASTNode clone0(AST target) {
        ...
        final Block result = new Block(this.getStart(), this.getEnd(), target,
                statements, this.isCurly());
        return result;
    }
