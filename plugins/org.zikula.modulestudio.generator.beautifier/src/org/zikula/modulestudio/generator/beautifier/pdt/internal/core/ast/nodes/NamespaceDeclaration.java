...
public class NamespaceDeclaration extends Statement {
    ...
    public void addStatement(Statement statement) {
        final Block body = getBody();
        body.statements().add(statement);

        final int statementEnd = statement.getEnd();
        final int bodyStart = body.getStart();
        body.setSourceRange(bodyStart, statementEnd - bodyStart + 1);

        final int namespaceStart = getStart();
    ...
    public void childrenAccept(Visitor visitor) {
        final NamespaceName name = getName();
        if (name != null) {
            name.accept(visitor);
        }
        final Block body = getBody();
    ...
    ASTNode clone0(AST target) {
        ...
        final NamespaceDeclaration result = new NamespaceDeclaration(
                this.getStart(), this.getEnd(), target, name, body, bracketed);
        return result;
    }
