...
public class PHPTextSequenceUtilities {
    ...
    public static TextSequence getStatement(int offset,
        ...
        if (tRegion != null
            ...
            final IPhpScriptRegion phpScriptRegion = (IPhpScriptRegion) tRegion;
            ...
            try {
                ...
                if (textSequence.toString().endsWith("(")) {
                    textSequence = textSequence.subTextSequence(0,
    ...
    private static TextSequence removeComments(TextSequence textSequence) {
        FIND_COMMENT_START: while (true) {
            final int commentStartPosition = getCommentStartIndex(textSequence);
            if (commentStartPosition > -1) {
                final String startCommentString = textSequence.subSequence(
                        commentStartPosition, commentStartPosition + 2)
                        .toString();
                if (startCommentString.equals(START_COMMENT)) {
                    // we are inside comment.
                    final Matcher commentEndMatcher = COMMENT_END_PATTERN
                            .matcher(textSequence);
                    final boolean foundEnd = commentEndMatcher
                            .find(commentStartPosition);
                    if (foundEnd) {
                        final int commentEndPosition = commentEndMatcher.end();
    ...
    private static int getCommentStartIndex(TextSequence textSequence) {
        final Matcher commentStartMatcher = COMMENT_START_PATTERN
                .matcher(textSequence);
        int start = 0;
        while (commentStartMatcher.find(start)) {
            final String currentType = TextSequenceUtilities.getType(
                    textSequence, commentStartMatcher.start());
    ...
    public static int getMethodEndIndex(CharSequence textSequence, int offset) {
        final int length = textSequence.length();
        while (offset < length
    ...
    public static int isInFunctionDeclaration(TextSequence textSequence) {
        final Matcher matcher = FUNCTION_PATTERN.matcher(textSequence);
        // search for the 'function' word.
        while (matcher.find()) {
            // verify char before 'function' word.
            final int functionStart = matcher.start();
            if (functionStart != 0
    ...
    public static int isInClassDeclaration(TextSequence textSequence) {
        final Matcher matcher = CLASS_PATTERN.matcher(textSequence);
        // search for the 'class' or 'interface words.
        while (matcher.find()) {
            // verify char before start.
            final int startOffset = matcher.start();
            if (startOffset != 0
                    && Character.isJavaIdentifierStart(textSequence
    ...
    public static int readNamespaceEndIndex(CharSequence textSequence,
        ...
        while (startPosition < length) {
            final char ch = textSequence.charAt(startPosition);
            if (!Character.isLetterOrDigit(ch) && ch != '_') {
                if (ch == '\\') {
                    if (onBackslash) {
                        break;
    ...
    public static ISourceRange getEnclosingIdentifier(
        ...
        final int start = readIdentifierStartIndex(textSequence, pos, true);
        final int end = readIdentifierEndIndex(textSequence, pos, true);
    ...
    public static int readForwardUntilDelim(CharSequence textSequence,
        ...
        int rv = startPosition;
        for (; rv < textSequence.length(); rv++) {
            final char c = textSequence.charAt(rv);
    ...
    public static int getPrivousTriggerIndex(CharSequence textSequence,
        ...
        for (; rv > 0; rv--) {
            final char currChar = textSequence.charAt(rv - 1);
