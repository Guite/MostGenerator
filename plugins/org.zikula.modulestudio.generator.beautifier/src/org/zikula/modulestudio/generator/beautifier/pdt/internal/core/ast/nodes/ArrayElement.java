...
public class ArrayElement extends ASTNode {
    ...
    public static final ChildPropertyDescriptor KEY_PROPERTY = new ChildPropertyDescriptor(
            ArrayElement.class, "key", Expression.class, OPTIONAL, CYCLE_RISK); //$NON-NLS-1$
    ...
    public ArrayElement(int start, int end, AST ast, Expression key,
            Expression value) {
        super(start, end, ast);
        if (value == null) {
    ...
    ASTNode clone0(AST target) {
        ...
        final ArrayElement result = new ArrayElement(this.getStart(),
                this.getEnd(), target, key, value);
        return result;
    }
