...
class Atom {
    ...
    def private atomView(Entity it, String appName) '''
        ...
        «IF app.targets('1.3.x')»
            ...
            {/if}
        «ENDIF»
        «IF app.targets('1.3.x')»{«appName.formatForDB»TemplateHeaders contentType='application/atom+xml'}«ENDIF»<?xml version="1.0" encoding="{charset assign='charset'}{if $charset eq 'ISO-8859-15'}ISO-8859-1{else}{$charset}{/if}" ?>
        ...
        {if $numItems}
        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:«IF app.targets('1.3.x')»{modurl modname='«appName»' type=$lct func='«defaultAction»' ot='«name.formatForCode»'«IF hasActions('display')» «routeParamsLegacy('items[0]', true, true)»«ENDIF»}«ELSE»{route name="«appName.formatForDB»_«name.formatForDB»_`$routeArea`«IF hasActions('display')»display«ELSE»«IF hasActions('view')»view«ELSE»index«ENDIF»«ENDIF»"«IF hasActions('display')» «routeParams('items[0]', true)»«ENDIF»}«ENDIF»{/capture}
            <id>{$uniqueID}</id>
        ...
        «ELSE»
            <link rel="alternate" type="text/html" hreflang="{lang}" href="{route name="«appName.formatForDB»_«name.formatForDB»_`$routeArea`«IF hasActions('index')»index«ELSEIF hasActions('view')»view«ELSE»«app.getAdminAndUserControllers.map[actions].flatten.toList.head.name.formatForCode»«ENDIF»" absolute=true}" />
        «ENDIF»
        ...
        {foreach item='«objName»' from=$items}
            <entry>
                ...
                «ELSE»
                    <link rel="alternate" type="text/html" href="{route name="«appName.formatForDB»_«name.formatForDB»_`$routeArea`«defaultAction»"«IF hasActions('display')» «routeParams(objName, true)»«ENDIF» absolute=true}" />
                    {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$«objName».createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{route name="«appName.formatForDB»_«name.formatForDB»_`$routeArea`«defaultAction»"«IF hasActions('display')» «routeParams(objName, true)»«ENDIF»}{/capture}
                «ENDIF»
                <id>{$uniqueID}</id>
                «IF standardFields»
