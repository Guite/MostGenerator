...
class Csv {
    ...
    def private csvView(Entity it, String appName, Controller controller) '''
        ...
        «FOR field : getDisplayFields.filter(e|e.name != 'workflowState') SEPARATOR ';'»«field.headerLine»«ENDFOR»«IF geographical»«FOR geoFieldName : newArrayList('latitude', 'longitude')»;"«geoFieldName.toFirstUpper»"«ENDFOR»«ENDIF»;"Workflow state"
        ...
        {foreach item='«objName»' from=$items}
            «FOR field : getDisplayFields.filter(e|e.name != 'workflowState') SEPARATOR ';'»«field.displayEntry(controller)»«ENDFOR»«IF geographical»«FOR geoFieldName : newArrayList('latitude', 'longitude')»;"{$«name.formatForCode».«geoFieldName»|formatnumber:7}"«ENDFOR»«ENDIF»;"{$item.workflowState|«appName.formatForDB»ObjectState:false|lower}"
