...
class SimpleFields {
    ...
    // 1.4.x only
    def private fieldRow(EntityField it, String groupSuffix, String idSuffix) '''{{ form_row(form.«IF groupSuffix != ''»«groupSuffix».«ENDIF»«name.formatForCode»«idSuffix»«helpAttribute») }}'''

    def private helpAttribute(EntityField it) '''«IF !helpMessages.empty», {help: «IF helpMessages.length > 1»[«ENDIF»«helpMessages.join(', ')»«IF helpMessages.length > 1»]«ENDIF»«ENDIF»'''

    def private helpDocumentation(EntityField it) {
        val messages = newArrayList
        if (null !== documentation && documentation != '') {
            messages += '__(\'' + documentation.replace("'", '"') + '\')'
        }
        messages
    }

    def private dispatch helpMessages(EntityField it) {
        ...
        if (!range && (hasMin || hasMax)) {
            if (hasMin && hasMax) {
                if (minValue == maxValue) {
                    messages += '''__f('Note: this value must exactly be %value%.', ['%value%' => «minValue»])'''
                } else {
                    messages += '''__f('Note: this value must be between %minValue% and %maxValue%.', ['%minValue%' => «minValue», '%maxValue%' => «maxValue»])'''
                }
            } else if (hasMin) {
                messages += '''__f('Note: this value must be greater than %minValue%.', ['%minValue%' => «minValue»])'''
            } else if (hasMax) {
                messages += '''__f('Note: this value must be less than %maxValue%.', ['%maxValue%' => «maxValue»]')'''
            }
        }
        messages
    }
    ...
    def private dispatch helpMessages(StringField it) {
        ...
        if (null !== regexp && regexp != '') {
            messages += '''__f('Note: this value must«IF regexpOpposite» not«ENDIF» conform to the regular expression "%pattern%".', ['%pattern%' => '«regexp.replace('\'', '')»'])'''
        }
    ...
    def private dispatch helpMessages(ListField it) {
        ...
        if (multiple && min > 0 && max > 0) {
            if (min == max) {
                messages += '''__f('Note: you must select exactly %min% choices.', ['%min%' => «min»])'''
            } else {
                messages += '''__f('Note: you must select between %min% and %max% choices.', ['%min%' => «min», '%max%' => «max»])'''
            }
        }
    ...
    def private dispatch helpMessages(AbstractDateField it) {
        ...
        if (past) {
            messages += '''__('Note: this value must be in the past.')'''
        } else if (future) {
            messages += '''__('Note: this value must be in the future.')'''
        }
