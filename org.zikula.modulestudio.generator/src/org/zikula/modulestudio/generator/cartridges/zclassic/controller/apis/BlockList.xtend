...
class BlockList {
    ...
    def private modify(Application it) '''
        ...
        public function modify(«IF targets('1.3.x')»$blockinfo«ELSE»Request $request, array $properties«ENDIF»)
        {
            ...
            «ELSE»
                «IF hasCategorisableEntities»
                    $properties['isCategorisable'] = in_array($properties['objectType'], $this->categorisableObjectTypes);
                «ENDIF»
                $form = $this->createForm('«appNamespace»\Block\Form\Type\ItemListBlockType'«IF hasCategorisableEntities», $properties«ENDIF»);
                ...
                return $this->renderView('@«appName»/Block/itemlist_modify«templateExtension»', [
                    'form' => $form->createView()«IF hasCategorisableEntities»,
                    'isCategorisable' => $properties['isCategorisable']«ENDIF»
                ]);
            «ENDIF»
        }
    '''
