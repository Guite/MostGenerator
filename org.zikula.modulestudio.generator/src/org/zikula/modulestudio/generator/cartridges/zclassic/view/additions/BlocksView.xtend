...
class BlocksView {
    ...
    def private editTemplateObjectType(Application it) '''
        <div class="«IF targets('1.3.x')»z-formrow«ELSE»form-group«ENDIF»">
            <label for="«appName.toFirstLower»ObjectType"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{gt text='Object type'}:</label>
            «IF !targets('1.3.x')»
                <div class="col-sm-9">
            «ENDIF»
                ...
                <span class="«IF targets('1.3.x')»z-sub z-formnote«ELSE»help-block«ENDIF»">{gt text='If you change this please save the block once to reload the parameters below.'}</span>
            «IF !targets('1.3.x')»
                </div>
            «ENDIF»
        </div>
        ...
        {if $catIds ne null && is_array($catIds)}
            ...
            {foreach key='propertyName' item='propertyId' from=$catIds}
                <div class="«IF targets('1.3.x')»z-formrow«ELSE»form-group«ENDIF»">
                    ...
                    {/if}
                    <label for="{$categorySelectorId}{$propertyName}"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{$categoryLabel}</label>
                    «IF !targets('1.3.x')»
                        <div class="col-sm-9">
                    «ELSE»
                        &nbsp;
                    «ENDIF»
                        {selector_category name="`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='«appName»' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize«IF !targets('1.3.x')» cssClass='form-control'«ENDIF»}
                        <span class="«IF targets('1.3.x')»z-sub z-formnote«ELSE»help-block«ENDIF»">{gt text='This is an optional filter.'}</span>
                    «IF !targets('1.3.x')»
                        </div>
                    «ENDIF»
                </div>
    ...
    def private editTemplateSorting(Application it) '''
        <div class="«IF targets('1.3.x')»z-formrow«ELSE»form-group«ENDIF»">
            <label for="«appName.toFirstLower»Sorting"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{gt text='Sorting'}:</label>
            «IF !targets('1.3.x')»
                <div class="col-sm-9">
            «ENDIF»
    ...
    def private editTemplateAmount(Application it) '''
        <div class="«IF targets('1.3.x')»z-formrow«ELSE»form-group«ENDIF»">
            <label for="«appName.toFirstLower»Amount"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{gt text='Amount'}:</label>
            «IF !targets('1.3.x')»
                <div class="col-sm-9">
            «ENDIF»
                <input type="text" id="«appName.toFirstLower»Amount" name="amount" maxlength="2" size="10" value="{$amount|default:"5"}"«IF !targets('1.3.x')» class="form-control"«ENDIF» />
            «IF !targets('1.3.x')»
                </div>
            «ENDIF»
        </div>
    ...
    def private editTemplateTemplate(Application it) '''
        <div class="«IF targets('1.3.x')»z-formrow«ELSE»form-group«ENDIF»">
            <label for="«appName.toFirstLower»Template"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{gt text='Template'}:</label>
            «IF !targets('1.3.x')»
                <div class="col-sm-9">
            «ENDIF»
        ...
        <div id="customTemplateArea" class="«IF targets('1.3.x')»z-formrow z-hide«ELSE»form-group hidden«ENDIF»"«IF !targets('1.3.x')» data-switch="«appName.toFirstLower»Template" data-switch-value="custom"«ENDIF»>
            <label for="«appName.toFirstLower»CustomTemplate"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{gt text='Custom template'}:</label>
            «IF !targets('1.3.x')»
                <div class="col-sm-9">
            «ENDIF»
                <input type="text" id="«appName.toFirstLower»CustomTemplate" name="customtemplate" size="40" maxlength="80" value="{$customTemplate|default:''}"«IF !targets('1.3.x')» class="form-control"«ENDIF» />
    ...
    def private editTemplateFilter(Application it) '''
        <div class="«IF targets('1.3.x')»z-formrow z-hide«ELSE»form-group«ENDIF»">
            <label for="«appName.toFirstLower»Filter"«IF !targets('1.3.x')» class="col-sm-3 control-label"«ENDIF»>{gt text='Filter (expert option)'}:</label>
            «IF !targets('1.3.x')»
                <div class="col-sm-9">
            «ENDIF»
                <input type="text" id="«appName.toFirstLower»Filter" name="filter" size="40" value="{$filterValue|default:''}"«IF !targets('1.3.x')» class="form-control"«ENDIF» />
                «IF targets('1.3.x')»
