        ...
        fsa.generateFile(templateFile(controller, name, 'include_displayItemList' + (if (many) 'Many' else 'One')), '''
            ...
            «IF hasImageFieldsEntity»
                ...
                {if $item.«imageFieldName» ne '' && isset($item.«imageFieldName»FullPath)}
                    «IF leadingField != null»
                        <img src="{$item.«imageFieldName»|«app.appName.formatForDB»ImageThumb:$item.«imageFieldName»FullPath:50:40}" width="50" height="40" alt="{$item.«leadingField.name.formatForCode»|replace:"\"":""}" />
                    «ELSE»
                        <img src="{$item.«imageFieldName»|«app.appName.formatForDB»ImageThumb:$item.«imageFieldName»FullPath:50:40}" width="50" height="40" alt="{gt text='«name.formatForDisplayCapital»'|replace:"\"":""}" />
                    «ENDIF»
                {/if}
            «ENDIF»
            «IF many»
