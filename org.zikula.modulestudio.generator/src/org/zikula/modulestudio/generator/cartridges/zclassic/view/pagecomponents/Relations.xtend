...
class Relations {
    ...
    def displayItemList(Entity it, Application app, Controller controller, Boolean many, IFileSystemAccess fsa) {
        val templatePath = templateFile(controller, name, 'include_displayItemList' + (if (many) 'Many' else 'One'))
    ...
    def displayRelatedItems(JoinRelationship it, String appName, Controller controller, Entity relatedEntity) '''
        ...
        «IF controller.hasActions('edit')»
            ...
            «ENDIF»
            {checkpermission component='«appName»:«relatedEntity.name.formatForCodeCapital»:' instance="«relatedEntity.idFieldsAsParameterTemplate»::" level='ACCESS_«IF controller instanceof AdminController»ADMIN«ELSE»«IF relatedEntity.workflow == EntityWorkflowType::NONE»EDIT«ELSE»COMMENT«ENDIF»«ENDIF»' assign='mayManage'}
